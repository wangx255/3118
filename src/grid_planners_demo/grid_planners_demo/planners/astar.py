...
class AStar:
    ...
    def solve(self) -> bool:
        ...
        open_set = {self.start.get_key()}

        closed_set = set()

        g_score = {self.start.get_key(): 0.0}

        while open_list:
            current_f, current_node = heapq.heappop(open_list)
            current_key = current_node.get_key()

            if current_key in open_set:
...
class AStar:
    ...
    def solve(self) -> bool:
        ...
        open_list = []
        heapq.heappush(open_list, (0.0, self.start))
        open_set = {(self.start.x, self.start.y)}

        closed_set = set()

        g_score = {(self.start.x, self.start.y): 0.0}
        ...
        while open_list:
            ...
            for neighbor in neighbors:
                ...
                tentative_g = g_score[(current_node.x, current_node.y)] + current_node.calculate_distance(neighbor)
